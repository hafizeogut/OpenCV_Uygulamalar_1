...
while 1:
    ...
    font=cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(frame,"CLEAR ALL",(40,33),font,0.5,(0,0,0),2,cv2.LINE_AA)
    ...
    #Daha temiz bir görüntü elde ediliyorç
    mask=cv2.erode(mask,kernel=(5,5),iterations=1)
    mask=cv2.morphologyEx(mask,cv2.MORPH_OPEN,kernel=(5,5))
    
    #kalınlaştırılıyor
    mask=cv2.dilate(mask,kernel=(5,5),iterations=1)
    

    contours,_=cv2.findContours(mask,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    #nesneye bir çember çiziliyor ve merkezi ile ilgileniliyor
    center=None
    
    if len(contours)>0:
        #True Büyükten küçüğe doğru sırala.
        max_contours=sorted(contours,key=cv2.contourArea,reverse=True)[0]
        
        #Kontur alanı içerisine sınırlayıcı çember çiziliyor
        ((x,y),radius)=cv2.minEnclosingCircle(max_contours)
        cv2.circle(frame,(int(x),int(y)),int(radius),(255,0,255),3)
        
        #Merkez noktası bulunuyor
        Moment=cv2.moments(max_contours)
        
        #center x değeri                         #y değeri
        center=(int(Moment["m10"]/Moment["m00"]),int(Moment["m10"]/Moment["m00"]))        
        #y deeğri 65 den küçükse:
        if center[1]<=65:
            if 40 <=center[0]<=140:
                blue_points=[deque(maxlen=512)]
                green_points=[deque(maxlen=512)]
                ...
                #Kapsam kuralları
                #67 den itibaren  x ve y değerleri taranıyor.
                #oraya koyulan butonlar gereği zaten çizim yapılmıyor.
                paintWindow[67,:,:]=255
